...
public class LoginInterceptor  implements HandlerInterceptor {
	...
	public boolean preHandle(HttpServletRequest request, HttpServletResponse response,
        ...
        if (!flag){
        	/** 1.获取session中的用户  */
        	User user = (User) request.getSession().getAttribute(TableCommon.USER_SESSION);
        	/** 2.判断用户是否已经登录 */
        	if(user == null){
        		 /** 如果用户没有登录，跳转到登录页面 */
        		request.setAttribute("message", "Ada提醒你，请先登录再访问网站!>>>");
        		request.getRequestDispatcher(TableCommon.LOGIN).forward(request, response);
        		return flag;
        	}else{
        		 flag = true;
        	}
